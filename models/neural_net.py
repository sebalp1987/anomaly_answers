...
class NeuralNetwork(object):
    ...
    def __init__(self, n_cols, node_size=[100, 60, 40],
...
if __name__ == '__main__':
    ...
    nn = NeuralNetwork(n_cols=cols, activation='relu', prob_dropout=None)
    early_stopping_monitor = EarlyStopping(patience=2)
    nn.fit_model(x, y, learning_rate=0.001, loss_function='categorical_crossentropy', epochs=10,
                 batch_size=500, verbose=True, callback_list=[early_stopping_monitor], validation_data=[valid, y_valid])

    nn.predict_model(x_valid=valid, y_valid=y_valid)